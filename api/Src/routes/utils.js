...
async function getFlightsOneWay({
  departurePlace,
  arrivalPlace,
  departureDate,
  returningDate,
  ...
  const flights = data.legs.map((flight) => {
    return {
      ...
      departureAirportName: data.airports
        .map((airport) =>
          flight.departureAirportCode === airport.code ? airport.name : null
        )
        .filter((a) => a !== null),
      ...
      arrivalAirportName: data.airports
        .map((airport) =>
          flight.arrivalAirportCode === airport.code ? airport.name : null
        )
        .filter((a) => a !== null),
      ...
      airlinesNames: data.airlines
        .map((airline) =>
          flight.airlineCodes.includes(airline.code) ? airline.name : null
        )
        .filter((a) => a !== null),
      price: data.fares
        .map((fare) =>
      ...
      segments: flight.segments.map((segment) => {
        return {
          ...
          departureAirportName: data.airports
            .map((airport) =>
              segment.departureAirportCode === airport.code
                ? airport.name
                : null
            )
            .filter((a) => a !== null),
          arrivalAirportName: data.airports
            .map((airport) =>
              segment.arrivalAirportCode === airport.code ? airport.name : null
            )
            .filter((a) => a !== null),
        };
      }),
...
async function getFlightsRoundTrip({
  departurePlace,
  arrivalPlace,
  departureDate,
  returningDate,
  ...
  const flights = data.legs.map((flight) => {
    return {
      ...
      airlinesNames: data.airlines
        ...
        .filter((a) => a !== null),
      going:
        flight.id.split(":")[0] === `${departurePlace}-${arrivalPlace}`
          ? true
          : false,
      returning:
        flight.id.split(":")[0] === `${arrivalPlace}-${departurePlace}`
          ? true
          : false,
      tripId: data.trips
      ...
      price: data.fares
        .map((fare) =>
          fare.tripId.includes(flight.id.split(":").slice(1, -1).join("-"))
            ? fare.price.totalAmount
            : null
        )
        .filter((a) => a !== null)
